...
from entity_spider.config import storage
from entity_spider.core.storage import locks
from entity_spider.core.storage.file import File
from entity_spider.core.storage._os import safe_join
from entity_spider.core.storage.move import file_move_safe
from entity_spider.core.storage.encoding import filepath_to_uri, force_unicode
from entity_spider.core.storage.exceptions import (SuspiciousFileOperation,
                                                     RemovedInDjango110Warning,
                                                     ImproperlyConfigured)
from entity_spider.core.storage.encoding import get_valid_filename, force_unicode, \
    force_unicode
...
class MogileFSStorage(Storage):
    ...
    def __init__(self, base_url=storage.MEDIA_URL):

        # the MOGILEFS_MEDIA_URL overrides MEDIA_URL
        if hasattr(storage, 'MOGILEFS_MEDIA_URL'):
            self.base_url = storage.MOGILEFS_MEDIA_URL
        else:
            self.base_url = base_url

        for var in ('MOGILEFS_TRACKERS', 'MOGILEFS_DOMAIN',):
            if not hasattr(storage, var):
                raise ImproperlyConfigured, "You must define %s to use the MogileFS backend." % var

        self.trackers = storage.MOGILEFS_TRACKERS
        self.domain = storage.MOGILEFS_DOMAIN
