...
def get_weixin_id_by_authorized_user_id(userid):
    ...
    try :
        weixin_id =  session.query(Profile).get(userid)\
                                 .weixin_id\
                                 .strip()
        return weixin_id
    except Exception as e :
...
def crawl_weixin_single_article_mission(mission, authorized_user_id=None, update_cookie=False , retry_count=5):
    '''
    :param mission:
        {
            'author':
    ...
    try :
        ...
        crawl_weixin_single_article_mission.delay(mission,authorized_user_id,update_cookie=False)
    except Retry as e:
        if (retry_count -1) > 0:
            weixin_client.refresh_cookies()
            crawl_weixin_single_article_mission.delay(mission,authorized_user_id, retry_count=retry_count-1)
        else :
            logger.error('retry count reach 0 , giving up for misssion: %s' % miss)
...
def crawl_user_weixin_articles_by_authorized_user_id(authorized_user_id, update_cookie=False, retry_count=5):
    ...
    if not weixin_id:
        logger.error('can not find weixin id for authorized_user_id : %s' %authorized_user_id)
        return
    ...
    try:
        ...
        if (retry_count -1) > 0:
            weixin_client.refresh_cookies()
            crawl_user_weixin_articles_by_authorized_user_id.delay(authorized_user_id, update_cookie=False, retry_count=retry_count-1)
        else :
            logger.error('retry count reach 0 , giving up for user weixin  : %s' % authorized_user_id)
            return
